...
from poliastro_live.settings import BASE_DIR, DEBUG
from . import rlcompleter
from .models import Searches, User

sys.path.insert(0, os.path.join(os.getcwd(), 'poliastro/src'))
...
def init_printing(*args, **kwargs):
    print("To change the printing method of poliastro Live, use the settings" + \
...
VERBOSE_MESSAGE = """\
Warning: this shell runs with poliastro %(version)s and so examples pulled from
...
#
#     message = "Python console for poliastro (Python %s)\n" % python_version
#
#     if not quiet:
#         source = ""
#
